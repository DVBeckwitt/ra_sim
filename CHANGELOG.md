...
- **Simulation performance**
  ...
  - Added a standalone Fig. 2 HT/Parratt explorer script with live GUI parameter controls, curve visibility controls including an optional stitched Parratt + `HT/Qz^2` display curve, divergence-safe `HT/Qz^2`, and stitched Parratt-to-HT diagnostics. Fixed the stitch mode so its scale-fit overlap is computed on an internal grid when the visible `L` range is too narrow, skipped the raw Miceli branch unless that stitch branch is selected, and queued stitch recomputation when the GUI enables the stitched curve before initial curves finish loading.
