...
- **GUI and UX updates**
  - Fixed selected-Qr rod caked integration masks so Qr half-width changes refill the displayed ROI shape and isolated +/-90 deg NaN trace samples no longer drop the surrounding finite Qz/Qr slice.
  ...
  - Restored main detector and caked figure right-drag panning after stale left-drag suppression could consume the right-button press; canvas button handling now normalizes Matplotlib/Tk integer, enum, and string button values, and right-drag release commits preview limits before ending the live interaction while preserving wheel zoom.
