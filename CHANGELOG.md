...
- **Fitting and optimization**
  ...
  - Fixed live GUI caked geometry-fit source-row fallback so first-fit preflight logs unambiguous internal/UI background indices, validates live rows and current hit tables before fresh simulation, rejects stale row caches with explicit reasons, and reports late fresh-simulation status instead of appearing stuck.
