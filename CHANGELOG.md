...
- **GUI and UX updates**
  ...
  - Expanded `ra_sim/gui/structure_factor_pruning.py` with zero-arg runtime binding/callback factories for pruning-status refresh, Bragg-Qr filter application, and solve-q control traces, leaving `ra_sim/gui/runtime.py` with bound factory values plus current-value helpers.
  ...
  - Expanded `ra_sim/gui/peak_selection.py` to own the selected-peak HKL-pick toggle and raw-image click workflow, the Bragg/Ewald intersection analysis path, and the runtime binding/callback bundle for HKL-pick labels, mode toggles, selected-peak refresh, HKL-control selection, and canvas-click selection, leaving `ra_sim/gui/runtime.py` with cross-feature click-mode dispatch plus live config-factory wiring.
...
- **Tests**
  ...
  - Extended `tests/test_gui_structure_factor_pruning.py` with direct coverage for the extracted structure-factor pruning / solve-q runtime-binding, factory, and callback workflow.
  ...
  - Extended `tests/test_gui_peak_selection.py` with direct coverage for the extracted HKL-pick toggle, raw-image click selection, runtime binding/callback bundle, and selected-peak Bragg/Ewald intersection workflow in addition to the existing HKL lookup and selected-peak state helpers.
