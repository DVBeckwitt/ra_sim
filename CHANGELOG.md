...
- **Simulation performance**
  ...
  - Added an interactive save-folder chooser for background peak-fit reruns so accepted output files can be redirected to one user-selected directory while headless and explicit output overrides stay deterministic.
  - Moved the maintained background peak-fit final-figure destination prompt to the final figure-save boundary, retitled it as a figure output folder chooser, and limited the accepted selection to final figure files so diagnostic artifacts keep their normal output paths.
