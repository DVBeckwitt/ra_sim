...
- **Simulation performance**
  ...
  - Added a standalone Fig. 2 HT/Parratt explorer script with live GUI parameter controls, curve visibility controls including an optional piecewise Parratt + `HT/Qz^2` display curve, divergence-safe `HT/Qz^2`, and stitched Parratt-to-HT diagnostics. Fixed the stitch mode so its scale-fit overlap is computed on an internal grid when the visible `L` range is too narrow, skipped the raw Miceli branch unless that stitch branch is selected, queued stitch recomputation when the GUI enables the stitched curve before initial curves finish loading, and changed the stitched display to a hard piecewise Parratt-to-kinematic handoff using one automatic log-median `HT/Qz^2` scale estimator with fixed Bragg exclusions and no GUI auto-fit button. Parratt optical thickness is now synchronized to the integer HT stack thickness `d = N*c`; `--thickness-nm` remains the target used to choose `N`, while the GUI, curve metadata, stitch diagnostics, and CLI warning report target thickness, effective thickness, mismatch, and matching HT/Parratt fringe periods. CSV redraw mode now replaces any CSV-provided pure Parratt branch with a synchronized recompute on the loaded `L` grid. The default stitch cut now requires a continuous Parratt/HT overlap interval; fixed and pointwise best-match cuts remain explicit legacy/debug modes. Added optional CPU worker control for wavelength-sample recomputation with serial mode still the default.
  ...
  - Fixed saved ghost Qr/Qz marker redraw after simulation changes by clearing stale geometry-fit overlay records when source-row or manual-pick cache authority is invalidated.
  ...
  - Added automatic robust sideband scaling for caked-plane radial background subtraction when no explicit scale is provided, while preserving numeric scale and `mode=off` overrides.
  - Added an interactive save-folder chooser for background peak-fit reruns so accepted final figures and used-peaks note/CSV files can be redirected to one user-selected directory while headless and explicit output overrides stay deterministic.
  - Moved the maintained background peak-fit final-figure destination prompt to the final figure-save boundary, retitled it as a figure output folder chooser, and kept large diagnostic arrays and caches on their normal output paths.
  - Moved background peak-fit saved profile artifacts into per-state `profiles/` folders derived from the imported GUI-state filename, so multiple imported states can share one output root without overwriting profile CSV/JSON/MD files or profile figures.
  - Fixed direct guarded background peak-fit reruns so default `auto` save-folder selection cannot silently block in the child process; forced `popup` mode still opens the chooser and now logs before waiting.
  ...
  - Fixed detector-view manual Qr/Qz background-click picking so one active selected Qr/Qz group is used even when stale detector-picker cache rows still contain extra groups; the same release now refines the clicked background peak, assigns the branch by refined proximity across the selected group, and lets repeat clicks near the same branch replace that branch instead of forcing the other branch.
  - Fixed manual Qr/Qz point placement after zoom so detector and caked release events use the current mouse cursor pixels when backend data coordinates are stale, preserving click-derived refinement seeds instead of reusing the zoom anchor.
  - Fixed manual Qr/Qz caked acceptance diagnostics so final caked residuals are traced to their row sources, pair-audit/final metric disagreements are classified as `caked_acceptance_metric_inconsistent`, and gamma/Gamma fits fail closed when only locked non-dynamic predictions are available.
  - Fixed detector-origin manual Qr/Qz caked fit handoff so stale simulated caked source fields remain diagnostic and exact projector output supplies the predicted caked anchor through provider, measured, initial, and manual-point handoff rows.
  ...
  - Fixed HK=0 Qr-rod peak-edit imports so imported phi/2theta ROI values immediately refresh the specular editor profile, are applied when loading an edit file path directly, and do not drop saved nonzero parameters when the HK=0 edit is saved.
...
- **Fitting and optimization**
  ...
  - Fixed PbI2 Qr-rod marker editor edits so `HK=4 -` marker
    ...
    nonzero Delta-Qr/L/theta_i controls and HK=0 phi/2theta ROI controls remain
    the scope-changing paths.
  - Fixed PbI2 HK=0 Qr-rod editor phase isolation so specular phi/2theta ROI
  ...
  - Fixed PbI2 HK=0/specular Qr-rod support so qz bounds come from the accepted
    phi/2theta ROI while profile integration, editor preview, and `00L_region`
    detector figures stay on the same specular detector region.
  - Fixed missing real PbI2 HK=0 Qr-rod profiles by rebuilding `m=0`,
    `branch="qz"` rows from active `00L` markers, active phi/2theta ROI bounds,
    and detector Q maps independent of cached base profile rows;
  ...
  - Added a popup-mode detector Qr-region companion preview beside the Qr-rod peak marker editor in the parallel background peak-fit diagnostic script, explicitly showing the companion window, refreshing detector overlays live during nonzero Delta Qr/L/theta_i edits and HK=0 phi/2theta edits, and deferring expensive Delta Qr profile reintegration until slider release or accept.
  ...
  - Fixed PbI2 Qr-rod profile/editor defaults so HK=0/specular uses phi/2theta ROI controls with a log y axis, nonzero HK rods use `0.5 <= L <= 3.0` with linear y axes, the detector companion preview uses the same phase-specific scope, and configured-hidden rows such as `HK=7` stay out of editor/support/final plots.
  ...
  - Fixed the HK=0 Qr-rod editor phase so it seeds from the pre-editor real intensity profile, keeps that last valid specular profile when a ROI refresh returns only nonzero rows, and reports marker-only fallback explicitly when real HK=0 intensity rows are unavailable.
  ...
  - Restored the PbI2 final HK=0 integration row by seeding HK=0 markers from the same detector Qz map used for integration, deriving the active support from the specular phi/2theta ROI, and falling back to the detector-space specular ROI strip when the direct `Qr=0` band has no pixels.
  ...
  - Added optional pre-fit central-phi caked plane background reduction to the parallel background peak-fit diagnostic, with popup/environment controls, raw detector preservation, detector-evaluated plane/corrected detector diagnostics, and cache signatures that change with the subtraction policy.
  - Changed the background editor from Matplotlib widgets to a Tk/Pillow canvas popup so scale changes update the labeled `-90 <= phi <= 90` caked plane preview without Matplotlib redraw lag.
