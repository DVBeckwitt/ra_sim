...
- **GUI and UX updates**
  ...
  - Expanded `ra_sim/gui/peak_selection.py` to own the selected-peak HKL-pick toggle and raw-image click workflow plus the Bragg/Ewald intersection analysis path, including nearest-peak resolution, ideal-center snap selection, geometry/beam/mosaic request packaging, figure-open handling, and status text updates, leaving `ra_sim/gui/runtime.py` with cross-feature click-mode dispatch plus thin control-value delegation.
...
- **Tests**
  ...
  - Extended `tests/test_gui_peak_selection.py` with direct coverage for the extracted HKL-pick toggle, raw-image click selection, and selected-peak Bragg/Ewald intersection workflow in addition to the existing HKL lookup and selected-peak state helpers.
