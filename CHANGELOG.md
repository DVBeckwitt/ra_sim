...
- **Fitting and optimization**
  ...
  - Fixed GUI saved-manual caked Qr fits that only vary detector tilts `gamma` and `Gamma`: trial simulated rows now reuse the prebuilt source cache while recomputing detector-tilt-aware caked angles, two-tilt saved states use the bounded ladder solve, and the final status distinguishes dynamic-fit RMS from full-beam overlay RMS. Validated on the Bi2Se3 and Bi2Te3 saved gamma/Gamma GUI states.
  ...
  - Changed PbI2 Qr-rod profile plotting to show raw central rod data, add the Qr sideband background back to available nonzero-rod fits, report marker/L and Qz-baseline cancellation checks as diagnostics instead of overlay gates, use logarithmic intensity axes on every panel, and cap the displayed L range at 3.
