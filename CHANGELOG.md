...
- **GUI and UX updates**
  ...
  - Expanded `ra_sim/gui/qr_cylinder_overlay.py` with analytic Qr-cylinder overlay render config, cache-signature, and path-construction helpers, leaving `ra_sim/gui/runtime.py` with thin overlay cache/status wiring around the extracted Bragg-Qr runtime helpers.
...
- **Tests**
  ...
  - Added `tests/test_gui_qr_cylinder_overlay.py` for the extracted analytic Qr-cylinder overlay config, signature, and detector/caked path-construction helpers.
  - Extended `tests/test_gui_geometry_q_group_manager.py` with direct coverage for the extracted geometry-fit Qr/Qz selector runtime binding/callback bundle plus the snapshot replacement/capture and preview-exclusion runtime helpers in addition to the side-effect and dialog workflow helpers.
