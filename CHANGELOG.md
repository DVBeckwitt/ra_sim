...
- **GUI and UX updates**
  ...
  - Made live caked Qr/Qz visual-source ledger output opt-in and cached manual-pick simulated-candidate refinement by simulation/exact-projection signature so repeated warm-cache caked clicks do not reprint trace rows or rerun full row refinement; lookup rebuild completion is tracked explicitly, failed lookup builders are retried, and no-signature direct calls clear stale skip metadata.
