...
- **Simulation performance**
  ...
  - Fixed detector-view manual Qr/Qz background-click picking so one active selected Qr/Qz group is used even when stale detector-picker cache rows still contain extra groups; the same release now refines the clicked background peak and assigns the branch by refined proximity.
