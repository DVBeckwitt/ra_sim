...
- **Fitting and optimization**
  ...
  - Recreated the parallel background peak-fit diagnostic script, restored final Qr-rod cache reuse by state filename, migrated the cache envelope away from pickle loading, separated `fit_l` from display-only `display_l` marker labels, drew used markers on Qr-rod profile figures, kept local peak snapping bounded to each marker window, fixed cache-hit specular support bounds, and normalized the script's Windows fit backend to threads to avoid `multiprocessing.spawn` re-running top-level diagnostic code.
