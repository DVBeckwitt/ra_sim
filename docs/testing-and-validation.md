...
- 2026-06-03: Standalone Fig. 2 HT/Parratt explorer is implemented locally. The script `scripts/plot_fig2_born_scaled_ht_near_zero.py` provides a live Tk/Matplotlib parameter GUI, curve visibility controls, divergence-safe `HT/Qz^2`, stitched Parratt-to-HT plotting modes, and stitch diagnostics. Bug/error status: the reviewed default `--plot-mode stitch` failure is fixed by computing the scale-fit overlap on an internal grid when the visible `L` range is too narrow; the raw Miceli `HT/Qz^2` branch is no longer computed unless `--stitch-branch raw-miceli-ht-q2` is selected. Migration/deprecation status: no package version, installed CLI, saved-state schema, config key, dependency, CI workflow, or artifact schema migration is required; generated figures and CSVs remain local artifacts. Shipping status: standalone research utility, no release/version bump or staged rollout required, rollback is a normal git revert. Validation status: `python -m pytest tests/test_plot_fig2_born_scaled_ht_near_zero.py -ra`, `python -m ruff check scripts/plot_fig2_born_scaled_ht_near_zero.py tests/test_plot_fig2_born_scaled_ht_near_zero.py`, `python -m ruff format --check scripts/plot_fig2_born_scaled_ht_near_zero.py tests/test_plot_fig2_born_scaled_ht_near_zero.py`, and `python -m compileall scripts/plot_fig2_born_scaled_ht_near_zero.py tests/test_plot_fig2_born_scaled_ht_near_zero.py` pass locally. Repo-wide `python -m ra_sim.dev check` is blocked by pre-existing formatting drift in `ra_sim/fitting/optimization.py`, `ra_sim/gui/_runtime/runtime_session.py`, and `ra_sim/test_tiers.py`.
