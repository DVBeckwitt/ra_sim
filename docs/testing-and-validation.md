...
| Validation probes | `scripts/plot_fig2_born_scaled_ht_near_zero.py` | `python scripts/plot_fig2_born_scaled_ht_near_zero.py --plot-mode stitch --show` | Figure PNG and curve CSV in the configured output directory; optional stitch diagnostics CSV/PNG. | Standalone Fig. 2 HT/Parratt explorer with live GUI mode, divergence-safe `HT/Qz^2`, stitched Parratt-to-HT diagnostics, and CSV redraw support. |
