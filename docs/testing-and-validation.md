...
- 2026-05-27: Simplification follow-up completed. Removed one tracked local New4 GUI-state snapshot and ignored future `new4_local_*.json` variants, kept documented New4 and background-diagnostic compatibility entry points, collapsed duplicate legacy background-subtraction CLI option registration, cached repeated source reads in large diagnostic/runtime source-inspection tests, collapsed exact duplicate geometry-test least-squares fakes, removed shadowed hBN geometry/bundle helper copies from `ra_sim/hbn.py`, and deleted the unused example fitting objective module. Public CLI names, compatibility wrapper names, saved-state behavior, and GUI workflow behavior are unchanged. Migration/review status: `ra_sim.hbn_geometry` remains the maintained hBN geometry implementation, `ra_sim.hbn` keeps compatibility wrappers for public helper names, no tracked references used the deleted example objective module, and review found no blocking correctness, security, performance, bloat, or test-quality issues; no avoidable new files or abstractions were added. Validation status: verified with targeted CLI, geometry, HBN, source-inspection, and runtime tests; `python -m compileall ra_sim tests`; and `python -m ra_sim.dev check`.
