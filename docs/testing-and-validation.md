...
- 2026-05-28: Manual Qr/Qz caked final-invariant review patch is committed as
  ...
  patch-level validation hardening, no release/version bump or staged rollout;
  rollback is a normal git revert. Review status: no new files, dependencies,
  wrapper modules, or public abstractions were added. Validation status:
...
- 2026-05-28: Manual Qr/Qz caked fit-space Tasks 4-5 are committed as `105ea5aa`.
  ...
  manual caked route with an explicit post-optimization `matched_pair_count == 0`
  fails as `manual_caked_route_invariant_violation` instead of completing as a
  deg-metric zero-match result. Review/API status: no public interface,
...
- 2026-05-28: GUI runtime Phase 4 extraction is committed as `3d9642aa`. The runtime update decision trace mutators and geometry-fit worker result/preflight-log helpers were moved into existing import-safe helper modules while preserving runtime behavior. Review follow-up made the update-action accessor private so the extraction does not create an avoidable public helper surface. Bug/error/feature status: internal cleanup only; no user-visible bug fix, GUI control, CLI behavior, saved-state schema, config key, artifact schema, dependency, or CI workflow changed. Migration/deprecation status: no migration required because the reviewed accessor was internal and uncommitted before this patch. Shipping status: no release/version bump or staged rollout required; rollback is a normal git revert. Validation status: focused runtime helper tests, GUI runtime import-safety tests, ruff, compileall, and `git diff --check` passed locally before commit.
...
- 2026-06-03: Standalone Fig. 2 HT/Parratt explorer is implemented locally. The script `scripts/plot_fig2_born_scaled_ht_near_zero.py` provides a live Tk/Matplotlib parameter GUI, curve visibility controls including an optional piecewise Parratt + `HT/Qz^2` display curve, divergence-safe `HT/Qz^2`, stitched Parratt-to-HT plotting modes, stitch diagnostics, and optional CPU workers for wavelength-sample recomputation. Bug/error status: the reviewed default `--plot-mode stitch` failure is fixed by computing the scale-fit overlap on an internal grid when the visible `L` range is too narrow; the raw Miceli `HT/Qz^2` branch is no longer computed unless `--stitch-branch raw-miceli-ht-q2` is selected; enabling the stitched curve before initial curves finish loading now queues the needed stitch recomputation automatically; and the stitched curve now uses a hard Parratt-to-kinematic handoff with the same automatic log-median `HT/Qz^2` scale used by the visible scaled curve. The Parratt optical slab thickness is now synchronized to the integer HT finite-stack thickness `d = N*c` by default, so the user-entered thickness is treated as a target used to choose `N`; for the default 50 nm Bi2Se3 target and `c = 28.636 A`, the synchronized effective thickness is 17 repeats, 48.681 nm, and `fringe_period_L_ht == fringe_period_L_parratt`. The GUI readout, curve metadata, stitch diagnostics, and non-GUI warning report target thickness, effective thickness, mismatch, stack layers, and matched fringe periods so this cannot look like exact 50 nm optics; CSV redraw mode also replaces any CSV-provided pure Parratt branch with a synchronized recompute on the loaded `L` grid. Bragg regions at `L=3,6,9` are masked from scale estimation rather than peak-fit, the GUI no longer exposes an auto-fit button or manual `HT/Qz^2` scale slider, the default cut mode now refuses to draw a stitched curve unless it finds a continuous Parratt/HT overlap interval, and `--workers`/the GUI CPU workers control keep serial mode as the default while allowing bounded threaded recompute across wavelength samples. Explicit fixed and pointwise best-match cut modes remain available for legacy/debug comparisons. Migration/deprecation status: no package version, installed CLI, saved-state schema, config key, dependency, CI workflow, or artifact schema migration is required; generated figures and CSVs remain local artifacts. Shipping status: standalone research utility, no release/version bump or staged rollout required, rollback is a normal git revert. Validation status: `python -m pytest tests/test_plot_fig2_born_scaled_ht_near_zero.py -ra`, `python -m pytest tests/test_testing_validation_index.py -ra`, `python -m ruff check scripts/plot_fig2_born_scaled_ht_near_zero.py tests/test_plot_fig2_born_scaled_ht_near_zero.py`, `python -m ruff format --check scripts/plot_fig2_born_scaled_ht_near_zero.py tests/test_plot_fig2_born_scaled_ht_near_zero.py`, and `python -m compileall scripts/plot_fig2_born_scaled_ht_near_zero.py tests/test_plot_fig2_born_scaled_ht_near_zero.py` pass locally. Repo-wide `python -m ra_sim.dev check` is blocked by pre-existing formatting drift in `ra_sim/fitting/optimization.py`, `ra_sim/gui/_runtime/runtime_session.py`, and `ra_sim/test_tiers.py`.
- 2026-05-27: Parratt low-Q stitch feature is implemented locally and ready for review. The GUI has an off-by-default `Parratt low-Q stitch (00L)` simulation option for finite-stack HT `00L` rods; the numerical helper uses active CIF optical constants, active lattice `c`, and stack thickness from layer count, while preserving high-Q HT intensity outside the blend window. Review follow-up removed the disabled-option hot-path copy, moved structure-model tests out of QR grouping tests, and added a regression for high-Q zero/negative intensity preservation. Migration/deprecation status: no CLI flag, config key, saved-state schema, dependency, CI workflow, artifact schema, or old behavior migration is required; existing saved GUI states load with the feature disabled unless they saved the new checkbox. Shipping status: feature flag is the GUI checkbox itself, default off, with normal git revert rollback. Validation status: focused Parratt/math/GUI/state tests and `python -m ra_sim.dev check` pass locally.
- 2026-05-27: Simplification follow-up completed and committed as `8e3295fe`. Removed one tracked local New4 GUI-state snapshot and ignored future `new4_local_*.json` variants, kept documented New4 and background-diagnostic compatibility entry points, collapsed duplicate legacy background-subtraction CLI option registration, cached repeated source reads in large diagnostic/runtime source-inspection tests, collapsed exact duplicate geometry-test least-squares fakes, removed shadowed hBN geometry/bundle helper copies from `ra_sim/hbn.py`, and deleted the unused example fitting objective module. Public CLI names, compatibility wrapper names, saved-state behavior, and GUI workflow behavior are unchanged. Migration/review status: `ra_sim.hbn_geometry` remains the maintained hBN geometry implementation, `ra_sim.hbn` keeps compatibility wrappers for public helper names, no tracked references used the deleted example objective module, and post-commit review found no actionable correctness, security, performance, bloat, test-quality, or unnecessary-new-code issues; no avoidable new files or abstractions were added. Shipping status: docs-only status follow-up after a behavior-preserving cleanup; no release, version bump, data migration, or rollout action is required, and rollback is a normal git revert of the cleanup commit if needed. Validation status: verified with targeted CLI, geometry, HBN, source-inspection, and runtime tests; `python -m compileall ra_sim tests`; and `python -m ra_sim.dev check`.
- 2026-05-27: Reviewed cleanup and geometry handoff patch is committed as `5cff06d6`. Geometry-fit async jobs now use validated Q-group snapshot rows before saved manual-pair fallback rows when live rows are empty, so disordered-phase source rows survive the handoff; coordinate-free Q-group metadata still falls back to saved manual pairs. Generated geometry-fit logs, temporary preflight JSON dumps, and the duplicate root PbI2 high-temperature CIF helper were removed or ignored. Migration status: the maintained helper remains `ra_sim.utils.pbi2_ht_shift_cif`; no tracked docs, CLI entries, or tests referenced the removed root script. Review status: no blocking correctness, security, performance, bloat, or test-quality findings; no new files or abstractions were added. Validation status: verified with `python -m pytest tests/test_geometry_fit_job_live_rows_handoff.py -ra`, `python -m pytest tests/test_gui_runtime_import_safe.py -ra`, `python -m pytest tests/test_pbi2_ht_shift_cif.py -ra`, and `python -m ra_sim.dev check`.
...
| Geometry recovery diagnostics | `tests/test_geometry_fit_coordinates.py` | `python -m pytest tests/test_geometry_fit_coordinates.py -ra` | Geometry-fit coordinate conversion and contract coverage. | Untiered; direct pytest or full suite. |
| Geometry recovery diagnostics | `tests/test_geometry_fit_job_selection.py` | `python -m pytest tests/test_geometry_fit_job_selection.py -ra` | Geometry-fit job selection and handoff behavior. | Untiered; direct pytest or full suite. |
| Geometry recovery diagnostics | `tests/test_geometry_fit_snapshot_contracts.py` | `python -m pytest tests/test_geometry_fit_snapshot_contracts.py -ra` | Geometry-fit snapshot contract helpers and regression fixtures. | Untiered; direct pytest or full suite. |
| Geometry recovery diagnostics | `tests/test_geometry_fit_worker.py` | `python -m pytest tests/test_geometry_fit_worker.py -ra` | Geometry-fit worker behavior and result handling. | Untiered; direct pytest or full suite. |
| Validation probes | `tests/test_plot_fig2_born_scaled_ht_near_zero.py` | `python -m pytest tests/test_plot_fig2_born_scaled_ht_near_zero.py -ra` | Standalone Fig. 2 HT/Parratt explorer defaults, shared HT/Parratt effective thickness, GUI parameter state, divergence-safe `HT/Qz^2`, stitch modes, diagnostics, and CSV redraw behavior. | Untiered; direct pytest or full suite. |
...
| Geometry fitting and projection validation | `tests/test_geometry_fit_dataset.py` | `python -m pytest tests/test_geometry_fit_dataset.py` | Geometry manual dataset collection, fit-space requirements, source labels, and saved-row truth handoff. | Untiered; direct pytest or full suite. |
...
| `tests/helpers/geometry_fit_snapshots.py` | Support helper | Imported by tests. | Shared geometry-fit snapshot fixtures and helpers. |
...
| Validation probes | `scripts/plot_fig2_born_scaled_ht_near_zero.py` | `python scripts/plot_fig2_born_scaled_ht_near_zero.py --plot-mode stitch-diagnostics --write-stitch-diagnostics --workers auto --show` | Figure PNG and curve CSV in the configured output directory; optional stitch diagnostics CSV/PNG. | Standalone Fig. 2 HT/Parratt explorer with live GUI mode, divergence-safe `HT/Qz^2`, automatic log-median `HT/Qz^2` scaling, continuous-overlap cut diagnostics, hard piecewise Parratt-to-HT handoff controls, optional CPU workers for wavelength samples, and CSV redraw support. |
