...
| Config, paths, and I/O | `tests/test_gui_state_io.py` | `python -m pytest tests/test_gui_state_io.py` | GUI state serialization, legacy load wrappers, background subtraction fields, and schema errors. | Untiered; direct pytest or full suite. |
...
| Geometry fitting and projection validation | `tests/test_diffuse_background.py` | `python -m pytest tests/test_diffuse_background.py` | Robust diffuse-background subtraction, peak/direct-beam masking, signed corrected arrays, and off-mode raw parity. | Untiered; direct pytest or full suite. |
