...
- Fixed: saved manual caked Qr/Qz fits that only vary detector tilts `gamma` and `Gamma` now keep the fast prebuilt source-row path while recomputing trial caked angles from the trial detector tilt. Status: validated through GUI harness runs for the Bi2Se3 and Bi2Te3 saved gamma/Gamma states; the completion text reports `Dynamic fit RMS` separately from the full-beam overlay RMS.
