...
- In the simulation options sampling/optics controls, `Parratt low-Q stitch (00L)` is off by default. When enabled, finite-stack Hendricks-Teller `00L` rods use the active CIF-derived optical constants and the active lattice `c` spacing to replace only the low-Q total-reflection region with a scaled air/material/air Parratt slab curve, then blend back to the HT rod. Non-`00L` rods, integer-Bragg debug mode, CLI flags, config keys, and saved-state schema are unchanged; the checkbox value is saved and restored with normal GUI state snapshots.
...
- The background peak-fit Figure 7 diagnostic prompts for a GUI state file from the current default state folder when launched interactively without an explicit state override. Its Qr-rod region editor now presents the nonzero-HK controls, HK=0 phi/2theta ROI controls, final-figure `m` visibility checkboxes, and the `Smooth` / `Adapt` controls for the plotted smoothed trace in one acceptance step so the final profile figure is driven by one accepted region state. `Adapt=0` preserves fixed Gaussian smoothing; larger values smooth lower-curvature regions more strongly while keeping sharper peak curvature closer to the base trace. Interactive runs without explicit output overrides offer a final figure output folder chooser at the save boundary; the selected folder receives final figures plus the used-peaks `.md`/`.csv`, including the combined `figure7_<sample>_qr_rod_qz_profiles` figure with HK=0 and drawable nonzero-HK rows. Detector-support completeness remains a diagnostic, not a reason to drop drawable nonzero profile rows from that compilation. Profile tables, notes, and support diagnostics also write under its `profiles/` child, and intermediate arrays/caches remain in `OUT_DIR`. Set `RA_SIM_HEADLESS=1` or explicit output-directory overrides for deterministic noninteractive paths, or set `RA_SIM_ALL_BACKGROUND_SAVE_DIR_EDIT_MODE=popup` to force the chooser.
...
- Fixed: after incident angle, detector, lattice, or other simulation-parameter changes, the selected HKL marker square is reselected from freshly rebuilt peak-overlay data before redraw. The marker should move with the current simulated target instead of remaining on stale cached overlay coordinates. Status: bug fixed; no new operator control, saved-state field, CLI flag, config key, or migration was added.
