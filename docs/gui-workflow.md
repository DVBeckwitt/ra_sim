...
- Fixed: saved manual caked Qr/Qz fits that only vary detector tilts `gamma` and `Gamma` now keep the fast prebuilt source-row path while using the same exact caked projector as import/redraw instead of a parallel detector-tilt projection shim. Status: validated through settled GUI harness runs for the Bi2Se3 and Bi2Te3 saved gamma/Gamma states; the completion text reports the visual overlay median distance against saved background picks and keeps `Dynamic fit RMS` separate from the full-beam overlay RMS.
