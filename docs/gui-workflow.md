...
- Enable `Include 6H Qr refs` beside the stacking controls when PbI2 stacking weight `w1` is nonzero and the manual picker should include packaged 6H reference Qr/Qz groups. The control is off by default, is saved in GUI state, and merges duplicate numeric Qr/Qz groups into one displayed and pickable group with combined detector candidates.
...
- Added: optional PbI2 6H reference Qr/Qz groups are generated from the packaged 6H CIF when `Include 6H Qr refs` is enabled and `w1` is nonzero; duplicate numeric groups merge before listing and manual picking.
