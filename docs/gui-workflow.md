...
- The background peak-fit Figure 7 diagnostic prompts for a GUI state file from the current default state folder when launched interactively without an explicit state override. Its Qr-rod region editor now presents the nonzero-HK controls, HK=0 phi/2theta ROI controls, and final-figure `m` visibility checkboxes in one acceptance step so the final profile figure is driven by one accepted region state. Direct Windows guarded process runs skip the final save-folder chooser in default `auto` mode; set `RA_SIM_ALL_BACKGROUND_SAVE_DIR_EDIT_MODE=popup` to force the chooser, or set explicit output-directory overrides for deterministic paths.
