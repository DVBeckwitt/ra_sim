...
- The background peak-fit Figure 7 diagnostic prompts for a GUI state file from the current default state folder when launched interactively without an explicit state override. Its Qr-rod region editor now presents the nonzero-HK controls, HK=0 phi/2theta ROI controls, final-figure `m` visibility checkboxes, and the `Smooth` / `Adapt` controls for the plotted smoothed trace in one acceptance step so the final profile figure is driven by one accepted region state. `Adapt=0` preserves fixed Gaussian smoothing; larger values smooth lower-curvature regions more strongly while keeping sharper peak curvature closer to the base trace. Direct Windows guarded process runs skip the final save-folder chooser in default `auto` mode; set `RA_SIM_ALL_BACKGROUND_SAVE_DIR_EDIT_MODE=popup` to force the chooser, or set explicit output-directory overrides for deterministic paths.
