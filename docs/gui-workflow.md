...
- Changing the standard integration region no longer rescales the main caked image. The radial 2theta and azimuthal phi 1D plots recompute, crop their displayed curves to the selected region, and autoscale to that cropped data. The Analyze `Log display` toggle also switches these 1D plot y-axes between linear and log intensity.
