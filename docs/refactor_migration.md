...
- Direct tests were added for extracted controller/state behavior.
  ...
  primary-CIF/CIF-weight helper coverage
...
- The next bounded GUI targets are the fit-geometry parameter checklist, the
  1D integration-range controls, and the remaining cross-feature runtime-owned
  helpers that are still built directly in `ra_sim.gui.runtime`.
