...
Feature status: shipped in repo. The tool writes ray-cloud and image-ROI shape
metrics, shape finite-difference long/matrix outputs, baseline shape metrics,
COM-vs-refined-max offsets, point/weight counts, provenance digests, and sparse
cloud validity statuses.

Bug/error status: fixed in repo. Shape evaluations now pass required baseline
pairs to source-row callbacks, weight selection falls back from non-positive
`intensity` to positive `weight`, valid COM/covariance rows are preserved when
only refined-max offset fields are NaN, and the default metric is
`ray_cloud_com`.
...
- External tracker status for issue #249 is not closed here because #249 covers
  broader geometric fitter recovery work.
