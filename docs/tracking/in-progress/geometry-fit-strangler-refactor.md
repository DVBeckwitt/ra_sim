...
## Patch G3 source candidate resolution handoff

What was done: moved source row keying, full-reflection row keying, source
locator payload matching, HKL/group/source-branch candidate filtering,
geometry-distance candidate scoring, tie handling, and legacy dense source
fallback into `ra_sim/gui/geometry_fit_dataset.py`. `geometry_fit.py` now keeps
thin local wrappers and provides `GeometrySourceCandidateDeps` callbacks for
current-view points, caked/display frame handling, live-row canonicalization,
and refined simulated override application.

Bug/error/feature status: this is an internal refactor slice. It preserves
dataset payload keys, nested `spec` keys, source identity fields, caked handoff
audit rows, solver inputs, saved-state behavior, CLI/env/debug behavior, UI
callbacks, and diagnostic log fields. Provider coverage remains in
`geometry_fit_dataset.py`. Dynamic trial row construction, caked dynamic
reanchor, source-row rebuild, final dataset assembly, worker/runtime code, and
optimizer code remain in `geometry_fit.py`.

Interface/API status: no public API changed. The new
`GeometrySourceCandidateDeps` dataclass is an internal module-boundary contract
so source-candidate helpers stay import-clean and do not depend on
`geometry_fit.py`, runtime, worker, optimizer, Tk, Matplotlib, saved-state
modules, or UI modules.

CI/shipping status: direct source-candidate helper tests, the full
`build_geometry_manual_fit_dataset` selector, the GUI workflow
caked/dataset/locked-Qr/coordinate/optimizer route pack, the geometry fitting
caked route pack, compileall, Ruff on touched files, and `git diff --check`
passed. No CI config, dependency, release-version, rollout, feature flag,
migration, or launch work is needed for this slice. `python -m ra_sim.dev
check` remains blocked only by pre-existing formatter drift in unrelated files.
...
## Patch G6 final dataset assembly contract guard

What was done: measured `build_geometry_manual_fit_dataset()` at 3,771 lines
after G5 and tightened the existing orientation-ready workflow test so it pins
the exact top-level dataset payload key inventory and exact nested `spec` key
inventory. The wrapper audit found no dead local wrappers that can be deleted
without changing callable handoff behavior; remaining thin wrappers are either
called locally or exported through the dataset `spec` callback surface.

Bug/error/feature status: this is a characterization and interface-guard
slice for the next dataset refactor decision. It does not move final dataset
assembly, source-row rebuild orchestration, worker/runtime code, optimizer
behavior, saved-state handling, CLI/env/debug behavior, UI behavior, solver
math, or diagnostic log fields.

Interface/API status: the public dataset dictionary and nested `spec`
dictionary remain unchanged. The new assertions make accidental payload drift
visible before any future final-assembly extraction.

Scope decision: final dataset assembly stays in `geometry_fit.py` for this
slice because it still owns audit row construction, audit line emission,
point-provider report construction, manual-point-pair handoff reconciliation,
and public payload packaging. Source-row rebuild stays local until a fresh
source-row-specific plan exists.

Validation status: the targeted
`test_build_geometry_manual_fit_dataset_assembles_orientation_ready_payload`
workflow test passed after the exact-key assertions were added.

## Patch G6 commit and launch status

What was done: completed the G6 contract-guard review and prepared an atomic
commit with only the tracking-note update and existing workflow-test contract
assertions staged.
...
workflow, optimizer behavior, or artifact format.

CI/CD status: no workflow automation or CI configuration changed. The relevant
local quality gates passed for the touched workflow test and tracking note; the
full dev check remains blocked only by documented unrelated formatter drift.
...
What was done: moved dynamic trial row signatures, dynamic-row
classification, stage lineage/summary diagnostics, target-key matching,
dynamic caked metadata marking, candidate sorting, analytic two-theta
fallback, caked point extraction, dynamic completion-row construction, and
candidate-pool supplementation into `ra_sim/gui/geometry_fit_dataset.py`.
`geometry_fit.py` keeps thin local wrappers and supplies
`GeometryDynamicTrialDeps` callbacks for source-coverage keys, group/HKL/branch
identity, caked point handling, JSONable diagnostics, simulated-peak provider
access, and current-view source-row projection.

Bug/error/feature status: this is an internal refactor slice. It preserves
dataset payload keys, nested `spec` keys, dynamic trial row metadata,
ghost-only required-pair behavior, `fit_qr_branch_key` construction,
source-coverage aliases, caked handoff audit rows, solver inputs, saved-state
behavior, CLI/env/debug behavior, UI callbacks, and diagnostic log fields.

Interface/API status: no public API changed. The new
`GeometryDynamicTrialDeps` dataclass is an internal module-boundary contract so
dynamic-trial helpers stay import-clean and do not depend on `geometry_fit.py`,
runtime, worker, optimizer, Tk, Matplotlib, saved-state modules, or UI modules.

Scope decision: `_qr_fit_trial_source_rows_builder()` remains in
`geometry_fit.py` because it still owns rebuild/source-row fallback ordering,
manual dataset bindings, active-parameter merging, stage aggregation, and final
trial-source payload assembly. Moving it in G4 would combine dynamic trial
helper extraction with orchestration and source-row rebuild behavior.

CI/shipping status: direct dynamic-trial helper tests, full dataset helper
tests, the `build_geometry_manual_fit_dataset` workflow selector, the GUI
workflow caked/dataset/locked-Qr/coordinate/optimizer route pack, the geometry
fitting caked route pack, compileall, Ruff on touched files, `git diff
--check`, and the dataset import-boundary check passed. No CI config,
dependency, release-version, rollout, feature flag, migration, or launch work
is needed for this slice. `python -m ra_sim.dev check` remains blocked only by
pre-existing formatter drift in unrelated files.
...
## Patch G4 commit and launch status

What was done: completed the G4 extraction review, kept the public
`geometry_fit.py` wrapper surface stable, and prepared the slice for an atomic
git commit with only the G4 code, tests, and tracking-doc updates staged.
...
Shipping status: launch risk is limited to the internal dataset extraction
boundary. No feature flag, staged rollout, release bump, or production launch
step is required; rollback is a normal commit revert.

## Patch G3 commit and launch status

What was done: completed the G3 extraction review, kept the public
`geometry_fit.py` wrapper surface stable, and prepared the slice for an atomic
git commit with only the G3 code, tests, and tracking-doc updates staged.

Bug/error/feature status: this remains an internal refactor feature slice, not
a user-visible behavior fix. No geometry-fit result, diagnostic schema,
saved-state, config, CLI, or GUI workflow change is intended.
...
Shipping status: launch risk is limited to the internal dataset extraction
boundary. No feature flag or staged rollout is required; rollback is a normal
...
## Patch G2 provider coverage augmentation handoff

What was done: moved provider coverage augmentation from the local
`build_geometry_manual_fit_dataset()` helper into
`augment_source_rows_with_provider_coverage()` in
`ra_sim/gui/geometry_fit_dataset.py`. The moved behavior includes provider-row
materialization bookkeeping, fresh-row scoring and unique selection,
fresh-row promotion, locked identity field preservation, caked dynamic-row
metadata on promoted rows, point-missing diagnostics, and targeted performance
gate diagnostic updates.

Bug/error/feature status: this is an internal refactor slice. It preserves
dataset payload keys, nested `spec` keys, provider coverage diagnostics,
caked handoff audit rows, solver inputs, saved-state behavior, CLI/env/debug
behavior, UI callbacks, and diagnostic log fields. Source candidate resolution,
dynamic trial row construction, caked dynamic reanchor, final dataset assembly,
worker/runtime code, and optimizer code remain in `geometry_fit.py`.

Interface/API status: no public API changed. The new
`GeometryProviderCoverageDeps` dataclass is an internal module-boundary
contract so the dataset helper stays import-clean and does not depend on
`geometry_fit.py`, runtime, worker, optimizer, Tk, Matplotlib, or saved-state
modules.

CI/shipping status: direct provider coverage helper tests, the full
`build_geometry_manual_fit_dataset` selector, the GUI workflow
caked/dataset/locked-Qr/coordinate/optimizer route pack, the geometry fitting
caked route pack, compileall, and Ruff on touched files passed. No CI config,
dependency, release-version, rollout, feature flag, migration, or launch work
is needed for this slice. `python -m ra_sim.dev check` remains blocked only by
pre-existing formatter drift in unrelated files.
...
- Patch G2 moved provider coverage augmentation, fresh-row matching, and
  fresh-row promotion into `ra_sim/gui/geometry_fit_dataset.py`; source
  candidate resolution, dynamic trial rows, caked dynamic reanchor, and final
  dataset assembly remain in `ra_sim/gui/geometry_fit.py`.
- Patch G3 moved source candidate resolution and legacy dense source fallback
  into `ra_sim/gui/geometry_fit_dataset.py`; dynamic trial rows, caked dynamic
  reanchor, source-row rebuild, and final dataset assembly remain in
  `ra_sim/gui/geometry_fit.py`.
- Patch G4 moved dynamic trial row helper behavior into
...
- Patch G2 validation passed: direct provider coverage helper tests, full
  `build_geometry_manual_fit_dataset` selector, GUI workflow
  caked/dataset/locked-Qr/coordinate/optimizer route pack, geometry fitting
  caked route pack, compileall, Ruff on touched files, and `git diff --check`.
  `python -m ra_sim.dev check` remains blocked only by pre-existing formatter
  drift in unrelated files.
- Patch G3 validation passed: direct source-candidate helper tests, full
  `build_geometry_manual_fit_dataset` selector, GUI workflow
  caked/dataset/locked-Qr/coordinate/optimizer route pack, geometry fitting
  caked route pack, compileall, Ruff on touched files, and `git diff --check`.
  `python -m ra_sim.dev check` remains blocked only by pre-existing formatter
...
- Patch G5 validation passed: direct caked dynamic reanchor helper tests, full
  ...
  ra_sim.dev check` remains blocked only by pre-existing formatter drift in
  unrelated files.
- Patch G6 validation passed: targeted
  `test_build_geometry_manual_fit_dataset_assembles_orientation_ready_payload`
