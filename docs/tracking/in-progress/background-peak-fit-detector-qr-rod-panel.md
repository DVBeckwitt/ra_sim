...
- Each Qr-rod peak marker now has an editable final-figure title. Blank titles
  fall back to `L=<display_l>`, and the editor `Label` text box sets the exact
  text used when the final Qr-rod figure is drawn.
...
- Targeted Qr-rod marker edit coverage for per-rod marker replacement,
  ...
  `HK=0` specular marker inclusion, and editor call ordering before the
  joint-fit cache lookup.
...
- Visual acceptance still needs manual script-output review: grayscale detector
  ...
  popup appearing on an interactive backend, edited peak titles appearing in
  the final Qr-rod figure, no misleading mixed-target rods, and reasonable
  `curve_distance_px` values.
