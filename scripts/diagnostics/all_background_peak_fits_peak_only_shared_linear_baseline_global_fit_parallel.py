...
def hk0_qz_marker_count(marker_table: object) -> int:
    table = pd.DataFrame(marker_table).copy()
    if table.empty or not {"m", "branch", "qz_marker"}.issubset(table.columns):
        return 0
    m_values = pd.to_numeric(table["m"], errors="coerce").to_numpy(dtype=np.float64)
    qz_values = pd.to_numeric(table["qz_marker"], errors="coerce").to_numpy(dtype=np.float64)
...
def rod_profile_has_real_hk0_qz_rows(rod_profile_table: object) -> bool:
    table = pd.DataFrame(rod_profile_table).copy()
    required = {"m", "branch", "qz_center", "pixel_count"}
...
def qr_rod_pre_editor_stage_is_valid(stage_payload: dict[str, object] | None) -> bool:
    ...
    specular_markers = pd.DataFrame(stage_payload.get("specular_l_marker_table", pd.DataFrame()))
    if hk0_qz_marker_count(specular_markers) > 0 and not rod_profile_has_real_hk0_qz_rows(
        rod_profile_table
    ):
        return False
    return (
...
def qz_l_linear_coeff_from_marker_rows(
    ...
    *,
    ...
    if ref_qz.size >= 2 and float(np.nanmax(ref_qz)) > float(np.nanmin(ref_qz)):
        if np.any(np.diff(ref_l) < -1.0e-6) or float(np.nanmax(ref_l)) <= float(np.nanmin(ref_l)):
            return fallback_coeff
...
def specular_qz_bounds_for_l_window(
    ...
    *,
    l_min: float = 0.0,
    l_max: float = 8.0,
    lattice_c: object = np.nan,
    positive_qz_min: float = 0.0,
) -> tuple[float, float] | None:
    marker_table = pd.DataFrame(marker_rows).copy()
    if not marker_table.empty and "qz_marker" in marker_table:
        slope, intercept = qz_l_linear_coeff_from_marker_rows(
            marker_table,
            fallback=(np.nan, np.nan),
        )
        if np.isfinite(slope) and np.isfinite(intercept) and float(slope) > 1.0e-12:
            l_lo, l_hi = sorted((float(l_min), float(l_max)))
            qz_l_lo = (l_lo - float(intercept)) / float(slope)
            qz_l_hi = (l_hi - float(intercept)) / float(slope)
            if np.isfinite(qz_l_lo) and np.isfinite(qz_l_hi):
                qz_lo, qz_hi = sorted((float(qz_l_lo), float(qz_l_hi)))
                qz_lo = max(qz_lo, float(positive_qz_min))
                if np.isfinite(qz_lo) and np.isfinite(qz_hi) and qz_hi > qz_lo:
                    return float(qz_lo), float(qz_hi)
    lattice_c_value = as_float(lattice_c, np.nan)
    if np.isfinite(lattice_c_value) and lattice_c_value > 0.0:
        l_lo, l_hi = sorted((float(l_min), float(l_max)))
        qz_window_lo = active_lattice_qz_value_for_l(l_lo, lattice_c_value)
        qz_window_hi = active_lattice_qz_value_for_l(l_hi, lattice_c_value)
        if np.isfinite(qz_window_lo) and np.isfinite(qz_window_hi):
            qz_lo, qz_hi = sorted((float(qz_window_lo), float(qz_window_hi)))
            qz_lo = max(qz_lo, float(positive_qz_min))
            if np.isfinite(qz_lo) and np.isfinite(qz_hi) and qz_hi > qz_lo:
                return float(qz_lo), float(qz_hi)
    qz = np.asarray(qz_values, dtype=np.float64)
    ...
    if qz.size < 2:
        return None
    qz_lo = max(float(np.nanmin(qz)), float(positive_qz_min))
    qz_hi = float(np.nanmax(qz))
    if not (np.isfinite(qz_lo) and np.isfinite(qz_hi)) or qz_hi <= qz_lo:
        return None
    return float(qz_lo), float(qz_hi)
...
def qr_rod_profile_cache_has_final_fit(
    ...
    if marker_table.empty or not {"qz_marker", "fit_l", "display_l"}.issubset(marker_table.columns):
        return False
    if hk0_qz_marker_count(marker_table) > 0 and not rod_profile_has_real_hk0_qz_rows(
        rod_profile_table
    ):
        return False
...
def qr_rod_editor_phase_matches(m_value: object, editor_phase: object = "all") -> bool:
    ...
    if phase in {"specular", "hk0", "00l"}:
        return m_int == 0
    raise ValueError(f"unknown Qr-rod editor phase: {editor_phase!r}")
...
def qr_rod_editor_phase_title(editor_phase: object = "all") -> str:
    ...
    if phase in {"specular", "hk0", "00l"}:
        return "Qr rod peak marker editor - HK=0 / 00L"
    return "Qr rod peak marker editor"


def merge_qr_rod_editor_phase_table(
    base_table: object,
    phase_table: object,
    *,
    editor_phase: object = "all",
) -> pd.DataFrame:
    phase = str(editor_phase if editor_phase is not None else "all").strip().lower()
    updated = pd.DataFrame(phase_table).copy()
    if phase in {"", "all"}:
        return updated.reset_index(drop=True)
    base = pd.DataFrame(base_table).copy()
    if base.empty or "m" not in base:
        return updated.reset_index(drop=True)
    if updated.empty or "m" not in updated:
        return base.reset_index(drop=True)

    def phase_mask(table: pd.DataFrame) -> np.ndarray:
        m_values = pd.to_numeric(table["m"], errors="coerce").to_numpy(dtype=np.float64)
        return np.asarray(
            [
                np.isfinite(m_value) and qr_rod_editor_phase_matches(int(m_value), phase)
                for m_value in m_values
            ],
            dtype=bool,
        )

    updated_phase_mask = phase_mask(updated)
    if not np.any(updated_phase_mask):
        return base.reset_index(drop=True)
    return pd.concat(
        [base.loc[~phase_mask(base)], updated.loc[updated_phase_mask]],
        ignore_index=True,
        sort=False,
    ).reset_index(drop=True)


def qr_rod_editor_profile_rows_from_markers(marker_table: object) -> pd.DataFrame:
    table = pd.DataFrame(marker_table).copy()
    required = {"m", "branch", "qz_marker"}
    if table.empty or not required.issubset(table.columns):
        return pd.DataFrame(columns=["m", "branch", "qz_center", "background_density"])
    rows = table.loc[:, ["m", "branch", "qz_marker"]].copy()
    rows["qz_marker"] = pd.to_numeric(rows["qz_marker"], errors="coerce")
    rows = rows[np.isfinite(np.asarray(rows["qz_marker"], dtype=np.float64))].copy()
    if rows.empty:
        return pd.DataFrame(columns=["m", "branch", "qz_center", "background_density"])
    rows["qz_center"] = np.asarray(rows["qz_marker"], dtype=np.float64)
    rows["background_density"] = 1.0
    return (
        rows.drop(columns=["qz_marker"])
        .drop_duplicates(subset=["m", "branch", "qz_center"], keep="first")
        .reset_index(drop=True)
    )
...
def show_qr_rod_peak_marker_popup(
    ...
    *,
    ...
    marker_profiles = qr_rod_editor_profile_rows_from_markers(edited)
    if not marker_profiles.empty:
        marker_profiles = pd.DataFrame(
            [
                row
                for row in marker_profiles.to_dict("records")
                if qr_rod_editor_phase_matches(int(row["m"]), editor_phase)
            ],
            columns=marker_profiles.columns,
        )
    marker_fallback_groups: set[tuple[int, str]] = set()
    required_profile_columns = {"m", "branch", "qz_center", "background_density"}
    if profiles.empty or not required_profile_columns.issubset(profiles.columns):
        profiles = marker_profiles.copy()
        marker_fallback_groups = {
            (int(row["m"]), str(row["branch"]))
            for row in marker_profiles[["m", "branch"]].drop_duplicates().to_dict("records")
        }
    elif not marker_profiles.empty:
        existing_profile_groups = {
            (int(row["m"]), str(row["branch"]))
            for row in profiles[["m", "branch"]].drop_duplicates().to_dict("records")
        }
        missing_marker_profiles = [
            row
            for row in marker_profiles.to_dict("records")
            if (int(row["m"]), str(row["branch"])) not in existing_profile_groups
        ]
        if missing_marker_profiles:
            marker_fallback_groups = {
                (int(row["m"]), str(row["branch"])) for row in missing_marker_profiles
            }
            profiles = pd.concat(
                [profiles, pd.DataFrame(missing_marker_profiles)],
                ignore_index=True,
                sort=False,
            ).reset_index(drop=True)
    if any(
        group[0] == 0 and qr_rod_editor_phase_matches(group[0], editor_phase)
        for group in marker_fallback_groups
    ):
        print(
            "HK=0 editor using marker-only fallback profile; real intensity rows unavailable",
            flush=True,
        )
    ...
    window_title = qr_rod_editor_phase_title(editor_phase)
    ...
    if region_controls_enabled:
        ...
        current_l_max = as_float(region_control_state.get("l_max"), max(current_l_min + 1.0, 8.0))
        ...
        current_phi_min = as_float(
            region_control_state.get("phi_min"), globals().get("specular_phi_min_deg", -10.0)
        )
        current_phi_max = as_float(
            region_control_state.get("phi_max"), globals().get("specular_phi_max_deg", 10.0)
        )
        current_two_theta_min = as_float(
            region_control_state.get("two_theta_min"),
            globals().get("specular_theta_min_deg", 5.0),
        )
        ...
        if np.isfinite(current_phi_min):
            region_control_state["phi_min"] = float(current_phi_min)
        if np.isfinite(current_phi_max):
            region_control_state["phi_max"] = float(current_phi_max)
        if np.isfinite(current_two_theta_min):
    ...
    hk0_roi_layout_enabled = (
        region_controls_enabled
    ...
    closing_linked_qr_rod_figures = False

    def close_linked_qr_rod_figures(event=None) -> None:
        nonlocal closing_linked_qr_rod_figures
        if closing_linked_qr_rod_figures:
            return
        event_fig = getattr(getattr(event, "canvas", None), "figure", None)
        editor_phase_text = str(editor_phase or "all").strip().lower()
        if (
            event_fig is fig
            and editor_phase_text == "nonzero"
            and bool(result.get("_editor_action_close", False))
        ):
            return
        closing_linked_qr_rod_figures = True
        try:
            for linked_fig in [fig, *companion_figure_list]:
                if linked_fig is event_fig:
                    continue
                try:
                    plt.close(linked_fig)
                except Exception:
                    pass
        finally:
            closing_linked_qr_rod_figures = False
    ...
    def current_hk0_roi_bounds() -> tuple[float, float, float, float]:
        defaults = (
            ...
            globals().get("specular_phi_max_deg", 10.0),
            globals().get("specular_theta_min_deg", 5.0),
            globals().get("specular_theta_max_deg", 25.0),
        )
    ...
    def hk0_roi_callback_kwargs(callback: object) -> dict[str, float]:
        ...
        for key, value in (
            ...
            if callback_accepts_keyword(callback, key):
    ...
    def refresh_region_profile_table() -> bool:
        ...
        updated_m_values = pd.to_numeric(updated_table["m"], errors="coerce").to_numpy(
            dtype=np.float64
        )
        updated_phase_rows = np.asarray(
            [
                np.isfinite(m_value) and qr_rod_editor_phase_matches(int(m_value), editor_phase)
                for m_value in updated_m_values
            ],
            dtype=bool,
        )
        if not np.any(updated_phase_rows):
            region_control_state["profile_update_error"] = (
                "profile update returned no drawable rows for current editor phase"
            )
            editor_debug(
                "profile_update.rejected",
                rows=int(updated_table.shape[0]),
                editor_phase=str(editor_phase),
                phase_rows=0,
                columns=list(updated_table.columns),
            )
            return False
        updated_table = merge_qr_rod_editor_phase_table(
            profiles,
            updated_table,
    ...
    def marker_y_values(markers: np.ndarray, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        if markers.size == 0:
            return np.asarray([], dtype=np.float64)
        if x.size < 2:
            return np.ones(markers.shape, dtype=np.float64)
    ...
    def redraw(*, preserve_limits: bool = False) -> None:
        ...
        for group, ax in axes_by_group.items():
            ...
            if group in preserved_limits:
                x_limits, y_limits = preserved_limits[group]
                if all(np.isfinite(x_limits)) and float(x_limits[0]) != float(x_limits[1]):
                    ax.set_xlim(*x_limits)
                if all(np.isfinite(y_limits)) and float(y_limits[0]) != float(y_limits[1]):
                    ax.set_ylim(*y_limits)
        fig.canvas.draw_idle()
    ...
    def set_region_roi_bound(key: str, text: object) -> None:
        ...
        current_values = dict(
            zip(
                ("phi_min", "phi_max", "two_theta_min", "two_theta_max"),
                current_hk0_roi_bounds(),
            )
        )
        value = as_float(text, current_values.get(str(key), np.nan))
        if np.isfinite(value):
            region_control_state[str(key)] = float(value)
        clear_group_plot_cache()
        mark_region_profile_refresh_pending("hk0_roi")
        mark_detector_preview_refresh_pending()
        editor_debug("hk0_roi.submit", key=str(key), text=text)
    ...
    def import_peak_edits(_event) -> None:
        ...
        try:
            ...
            imported_groups = (
                [
                    (int(row["m"]), str(row["branch"]))
                    for row in edited[["m", "branch"]].drop_duplicates().to_dict("records")
                ]
                if not edited.empty and {"m", "branch"}.issubset(edited.columns)
                else []
            )
            selected["group"] = None
            selected["index"] = None
    ...
    def on_release(_event) -> None:
        ...
        try:
            ...
            restore_group_axis_limits(limits)
        if "marker_table" not in region_profile_refresh_sources:
    ...
    def on_key(event) -> None:
        ...
        if key in {"delete", "backspace"}:
            ...
            flush_detector_region_preview()
            result["accepted"] = True
            result["_editor_action_close"] = True
            plt.close(fig)
        elif key in {"escape"}:
    ...
    if region_controls_enabled:
        ...
        if roi_controls_active:
            ...
            for key, label, value, bounds in roi_specs:
                ...
                text_input_widgets.append(box)

    if roi_controls_active:
        button_axes = [
            fig.add_axes([0.77, 0.035, 0.060, 0.05]),
            fig.add_axes([0.835, 0.035, 0.060, 0.05]),
            fig.add_axes([0.900, 0.035, 0.060, 0.05]),
            fig.add_axes([0.770, 0.000, 0.090, 0.028]),
            fig.add_axes([0.870, 0.000, 0.090, 0.028]),
        ]
    else:
        button_axes = [
    ...
    if roi_controls_active:
        # HK=0 needs the ROI-scoped detector/profile state before the first pick.
        refresh_region_profile_table()
        refresh_detector_region_preview()
    redraw()
    show_companion_figures()
    print(f"opening {window_title}; click Accept to continue", flush=True)
    plt.show(block=True)
...
def caked_background_plane_from_image(
    ...
    *,
    ...
    if np.count_nonzero(fit_mask) < 3:
        ...
        fit_surface = np.full(image.shape, np.nan, dtype=np.float64)
        model_surface = np.full(image.shape, np.nan, dtype=np.float64)
        fit_surface[fit_mask] = image[fit_mask]
        model_surface[fit_mask] = model[fit_mask]
        return {
    ...
    if (
        ...
        return {
            ...
            **normalized,
            ...
            "caked_fit_surface": plane_payload["fit_surface"],
            "caked_model_surface": plane_payload["model_surface"],
    ...
    *,
    ...
    def update_preview(*, recompute_model: bool = False) -> None:
        ...
        if str(config["mode"]) == "radial" and scale > 0.0:
            ...
            fit_surface_caked_image = np.full_like(raw_caked_preview, np.nan, dtype=np.float64)
        update_panel_image("model", model_caked_image)
        update_panel_image("corrected", corrected_caked_image)
        update_panel_image("fit_surface", fit_surface_caked_image)
    ...
    try:
        ...
        add_image_panel(
            "fit_surface",
            "Plane fit surface (-90 <= phi <= 90)",
            1,
            1,
            detector_preview_image(
                np.full_like(raw_caked_preview, np.nan, dtype=np.float64),
                x_axis=caked_theta_preview,
                y_axis=caked_phi_preview,
                x_label="2theta (deg)",
                y_label="phi (deg)",
...
def detector_phi_two_theta_roi_mask(
    phi_map: object,
    two_theta_map: object,
    *,
    phi_min: object,
    phi_max: object,
    two_theta_min: object,
    two_theta_max: object,
) -> np.ndarray:
    phi = np.asarray(phi_map, dtype=np.float64)
    two_theta = np.asarray(two_theta_map, dtype=np.float64)
    if phi.shape != two_theta.shape:
        raise RuntimeError("detector phi and 2theta maps must have matching shapes")

    def finite_bound(value: object) -> float:
        try:
            return float(value)
        except (TypeError, ValueError):
            return float("nan")

    phi_lo, phi_hi = sorted((finite_bound(phi_min), finite_bound(phi_max)))
    theta_lo, theta_hi = sorted((finite_bound(two_theta_min), finite_bound(two_theta_max)))
    if not all(np.isfinite(value) for value in (phi_lo, phi_hi, theta_lo, theta_hi)):
    ...
    return (
        ...
        & (phi >= float(phi_lo))
        & (phi <= float(phi_hi))
        & (two_theta >= float(theta_lo))
        & (two_theta <= float(theta_hi))
    )
...
def profile_from_detector_qr_qz(
    ...
    *,
    ...
    profile_mask_override: np.ndarray | None = None,
    qr_integration_source_override: str | None = None,
) -> pd.DataFrame:
    ...
    if profile_mask_override is not None:
        ...
        if override_mask.shape != image.shape:
            return pd.DataFrame()
    ...
    fast_payload = None
    if override_mask is None and edges_uniform:
    ...
    base = base_common & (
        override_mask if override_mask is not None else ((qr_map >= qr_lo) & (qr_map <= qr_hi))
    )
    ...
    if override_mask is None and bool(QR_ROD_TRANSVERSE_BACKGROUND_ENABLED):
        ...
        subtract = (
            nonempty
            & (sideband_counts >= int(QR_ROD_BG_MIN_SIDE_PIXELS))
            & np.isfinite(sideband_density)
        )
        if np.any(subtract):
    ...
    return pd.DataFrame(
        {
            ...
            "qr_integration_source": [
                str(qr_integration_source_override or "detector_mask_per_qz_bin")
...
def profile_from_detector_phi_two_theta_roi(
    bg: dict[str, object],
    rod: dict[str, object],
    *,
    branch_name: str,
    ...
    if not np.any(roi_mask):
        return pd.DataFrame()
    return profile_from_detector_qr_qz(
        bg,
        rod,
        branch_name=branch_name,
        branch_label=branch_label,
        ...
        phi_min=-180.0,
        phi_max=180.0,
        detector_q_maps=detector_q_maps,
        detector_phi_map=detector_phi_map,
        detector_solid_angle=detector_solid_angle,
        detector_two_theta_map=detector_two_theta_map,
        delta_qr_override=0.0,
        profile_mask_override=roi_mask,
        qr_integration_source_override="detector_phi_two_theta_roi_per_qz_bin",
    )
...
def rod_profile_marker_l_mapping_is_valid(
    ...
    *,
    ...
    if np.any(np.diff(l_values) < -1.0e-6) or np.nanmax(l_values) <= np.nanmin(l_values):
        return False
    qz_span = float(np.nanmax(qz) - np.nanmin(qz))
    l_span = float(np.nanmax(l_values) - np.nanmin(l_values))
    if str(globals().get("SAMPLE_STEM", "")).lower().startswith("pbi2") and int(m_value) != 0:
        try:
            min_span_ratio = float(globals().get("PBI2_NONZERO_MARKER_L_MIN_SPAN_RATIO", 0.5))
        except Exception:
            min_span_ratio = 0.5
        if qz_span > 0.0 and min_span_ratio > 0.0 and (l_span / qz_span) < min_span_ratio:
            return False
    ...
    if is_pbi2_nonzero and bool(background_subtraction_disabled):
        ...
        for candidate_column in ("joint_fit_density", "fit_density"):
            if candidate_column in table.columns:
                ...
                break
        reason = "pbi2_no_background_subtraction_debug" if model_column else "missing_model_density"
        return {
...
def detector_rotation_anchor_summary(
    ...
    if active_mask is None:
        ...
        selected = [
            point for idx, point in enumerate(points) if idx < mask.size and bool(mask[idx])
        ]
...
def specular_roi_signature_from_bounds(
    *,
    phi_min: object,
    phi_max: object,
    two_theta_min: object,
    ...
    return {
        "phi_min": round(float(phi_min), 9),
        "phi_max": round(float(phi_max), 9),
        "two_theta_min": round(float(two_theta_min), 9),
        "two_theta_max": round(float(two_theta_max), 9),
    }
...
SPECULAR_ROI_SIGNATURE = specular_roi_signature_from_bounds(
...
specular_detector_phi_mask = detector_phi_two_theta_roi_mask(
...
def specular_l_marker_rows_with_lattice_fallback(
    marker_rows: object, *, lattice_c: object, l_max: object, l_min: object = 0.0
) -> pd.DataFrame:
    table = pd.DataFrame(marker_rows).copy()
    l_min_value = as_float(l_min, 0.0)
    if not np.isfinite(l_min_value):
        l_min_value = 0.0
    ...
    for row in table.to_dict("records") if not table.empty else []:
        ...
        if np.isfinite(l_value) and (
            float(l_value) < float(l_min_value)
            or (np.isfinite(l_max_value) and float(l_value) > float(l_max_value))
        ):
            continue
        records.append(row)
    ...
    min_l = max(1, int(np.ceil(float(l_min_value))))
    for l_value in range(min_l, max_l + 1):
    ...
    return (
        out.sort_values(["_sort_l", "qz_marker"], kind="mergesort")
        .drop(columns=["_sort_l"])
        .reset_index(drop=True)
    )


def active_specular_marker_rows_for_l_window(
    marker_source: object, *, l_min: object, l_max: object
) -> pd.DataFrame:
    source = pd.DataFrame(marker_source).copy()
    required = pd.DataFrame(globals().get("specular_l_marker_table", pd.DataFrame())).copy()
    merged = marker_table_with_specular_l_markers(source, required)
    specular = pd.DataFrame()
    if not merged.empty and {"m", "branch", "qz_marker"}.issubset(merged.columns):
        m_values = pd.to_numeric(merged["m"], errors="coerce").to_numpy(dtype=np.float64)
        branch_values = merged["branch"].astype(str)
        specular = merged.loc[
            np.isfinite(m_values) & (m_values == 0.0) & (branch_values == "qz")
        ].copy()
...
def specular_l_marker_rows_for_background(
    ...
    return specular_l_marker_rows_with_lattice_fallback(
        ...
        l_max=qr_rod_l_bounds_for_group(
            0,
            (float(SPECULAR_QR_ROD_L_MIN), float(SPECULAR_QR_ROD_L_MAX)),
        )[1],
    )
...
def l_reference_rows(
    *, m_value: int, branch_value: str, marker_source: pd.DataFrame | None = None
    ...
    source = (
        globals().get("plot_marker_table", pd.DataFrame())
    ...
    if sub.empty:
        return sub
    if not rod_profile_marker_l_mapping_is_valid(
        source, m_value=int(m_value), branch_value=str(branch_value)
    ):
...
def rod_profile_table_for_l_window(
    ...
    *,
    ...
    specular_bounds: tuple[float, float] | None = None
    specular_l_lo = as_float(specular_l_min, np.nan)
    specular_l_hi = as_float(specular_l_max, np.nan)
    if np.isfinite(specular_l_lo) and np.isfinite(specular_l_hi):
        specular_l_lo, specular_l_hi = sorted((float(specular_l_lo), float(specular_l_hi)))
        if specular_l_hi > specular_l_lo:
            specular_bounds = (float(specular_l_lo), float(specular_l_hi))
    for (m_value, branch_value), sub in table.groupby(["m", "branch"], sort=False):
...
qr_rod_editor_profile_cache: dict[tuple[object, ...], pd.DataFrame] = {}
...
def recompute_qr_rod_region_profiles(
    ...
    *,
    ...
    phi_min_number = as_float(phi_min, specular_phi_min_deg)
    ...
    active_marker_source = (
        ...
        else pd.DataFrame(marker_table).copy()
    )
    active_marker_source = marker_table_with_specular_l_markers(
        active_marker_source,
    ...
    if qr_rod_editor_phase_matches(0, editor_phase):
        specular_profile = build_specular_qr_rod_profile(
            ...
            two_theta_max=theta_max_number,
        )
        if not specular_profile.empty:
            rebuilt_rows.append(specular_profile)
    ...
    try:
        ...
        def draw_preview_boundary(mask: np.ndarray, color: str, *, alpha: float) -> None:
            ...
            if not np.any((~boundary) & preview_shape_mask):
                return
            values = np.ma.masked_where(~preview_shape_mask, np.asarray(boundary, dtype=np.float64))
        ...
        def preview_qz_bounds_for_l_window(
            ...
            *,
            ...
            l_min_value, l_max_value = l_bounds
            qz_window_values = np.asarray([qz_lo, qz_hi], dtype=np.float64)
            if int(m_value) == 0:
                l_clipped_bounds = specular_qz_bounds_for_l_window(
                    qz_window_values,
                    marker_rows,
                    l_min=float(l_min_value),
                    l_max=float(l_max_value),
        ...
        def update_preview(
            *,
            ...
            _, specular_preview_qz_map, specular_preview_valid_q = specular_detector_q_maps
            specular_preview_roi_mask = detector_phi_two_theta_roi_mask(
                profile_detector_phi_map,
                profile_detector_two_theta_map,
                phi_min=phi_min_value,
                phi_max=phi_max_value,
            ...
            preview_specular_qz_values = np.asarray(
                specular_preview_qz_map[
                    preview_shape_mask
                    & specular_preview_valid_q
                    & specular_preview_roi_mask
                    & np.isfinite(specular_preview_qz_map)
                    & (specular_preview_qz_map > POSITIVE_QZ_MIN)
                ],
                dtype=np.float64,
            )
...
if np.isfinite(qr_rod_specular_phi_max_deg):
    specular_phi_max_deg = float(qr_rod_specular_phi_max_deg)
if np.isfinite(qr_rod_specular_theta_min_deg):
    specular_theta_min_deg = float(qr_rod_specular_theta_min_deg)
if np.isfinite(qr_rod_specular_theta_max_deg):
    specular_theta_max_deg = float(qr_rod_specular_theta_max_deg)
SPECULAR_ROI_SIGNATURE = specular_roi_signature_from_bounds(
    phi_min=specular_phi_min_deg,
    phi_max=specular_phi_max_deg,
    two_theta_min=specular_theta_min_deg,
...
if bool(qr_rod_region_editor_result.get("accepted", False)) and qr_rod_peak_edits_path:
    try:
        ...
        print(
            f"failed saving final Qr-rod peak edits={Path(qr_rod_peak_edits_path).expanduser()}: {exc}"
        )
...
if 0 not in hidden_plot_hk and (0, "qz") in drawable_profile_keys:
    ...
    if final_hk0_marker_count > 0:
        final_hk0_profile_rows = 0
        final_hk0_positive_pixel_rows = 0
        if not rod_profile_table.empty and {"m", "branch"}.issubset(rod_profile_table.columns):
            final_profile_m = pd.to_numeric(rod_profile_table["m"], errors="coerce").to_numpy(
                dtype=np.float64
            )
            final_hk0_mask = (
                np.isfinite(final_profile_m)
                & (final_profile_m == 0.0)
                & (rod_profile_table["branch"].astype(str) == "qz")
            )
            final_hk0_profile_rows = int(np.count_nonzero(final_hk0_mask))
            if "pixel_count" in rod_profile_table:
                final_profile_pixels = pd.to_numeric(
                    rod_profile_table["pixel_count"], errors="coerce"
                ).to_numpy(dtype=np.float64)
                final_hk0_positive_pixel_rows = int(
                    np.count_nonzero(
                        final_hk0_mask
                        & np.isfinite(final_profile_pixels)
                        & (final_profile_pixels > 0.0)
                    )
                )
        print(
            "skipped HK=0 final figure row: no drawable real m=0/qz profile rows "
            f"rows={final_hk0_profile_rows} "
            f"positive_pixel_rows={final_hk0_positive_pixel_rows} "
            f"marker_count={final_hk0_marker_count}"
        )
    ...
    *,
    ...
    def nudge_label(dx: float, dy: float) -> None:
        ...
        set_label_canvas_position(
            active_index(), float(x_value) + float(dx), float(y_value) + float(dy)
        )
...
if specular_visual_qz_bounds is not None:
    specular_roi_visual_mask = (
        detector_region_shape_mask
        & specular_detector_valid_q
        & specular_detector_phi_mask
        & np.isfinite(specular_detector_qz_map)
        & (specular_detector_qz_map >= float(specular_visual_qz_bounds[0]))
        & (specular_detector_qz_map <= float(specular_visual_qz_bounds[1]))
    )
    specular_delta_q_visual = {
        "band_fill_mask": specular_roi_visual_mask,
        "band_boundary_inner_visible": specular_roi_visual_mask,
        "band_boundary_outer_visible": specular_roi_visual_mask,
    }
