...
def main() -> None:
    ...
    def _detach(x):
        ...
        if isinstance(x, list):     return np.asarray([_detach(a) for a in x], dtype=object)
        if isinstance(x, np.ndarray): return np.ascontiguousarray(x.copy())
        return x

    script_dir = Path(__file__).resolve().parent
    out        = script_dir / "simulation.npz"

    arrays = {
        "image": _detach(image).astype(np.float32),
    }

    for i, tbl in enumerate(hit_tables):
